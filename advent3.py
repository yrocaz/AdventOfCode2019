    ...
    for i, _ in enumerate(wire):
        if wire[i][0] == 'R': # add x / move R
            ...
            for j in range(1, x+1):
                line_x = path[i][0] - j
                lines.append((line_x, path[i][1]))
        elif wire[i][0] == 'U': # add y / move UP
            y = int(wire[i][1:])
            new_y = path[i][1] + y
            path.append((path[i][0], new_y))
            for j in range(1, y+1):
                line_y = path[i][1] + j
                lines.append((path[i][0], line_y))
        elif wire[i][0] == 'D': # subtract x / move DOWN
            y = int(wire[i][1:])
            new_y = path[i][1] - y
            path.append((path[i][0], new_y))
            for j in range(1, y+1):
...
Path(wire1, path1, lines1)
Path(wire2, path2, lines2)

line_intersections = list(set(lines1[1:]) & set(lines2[1:])) # set() takes care of wire crossing itself
distance = min([abs(x)+abs(y) for x,y in line_intersections]) # distance from origin to closest intersection.
...
for i in line_intersections:
    print(f'{lines1.index(i)} + {lines2.index(i)} = {lines1.index(i) + lines2.index(i)}')
